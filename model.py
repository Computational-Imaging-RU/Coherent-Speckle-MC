...
def get_subdict(dictionary, prefix):
    """
    Extracts a subset of a dictionary based on a prefix.

    Args:
    - dictionary (dict): The dictionary to extract the subset from.
    - prefix (str): The prefix to filter the keys.

    Returns:
    - subdict (dict): The subset of the dictionary.
    """
    subdict = {}
...
class BatchLinear(nn.Linear, Module):
...
class FCBlock(nn.Module):
    ...
    def __init__(self, in_features, out_features, num_hidden_layers, hidden_features,
        ...
        # Modified MLP using nn.Linear
        self.net.append(Sequential(nn.Linear(in_features, hidden_features), nl))
        for i in range(num_hidden_layers):
            self.net.append(Sequential(nn.Linear(hidden_features, hidden_features), nl))
        if outermost_linear:
            self.net.append(Sequential(nn.Linear(hidden_features, out_features), nn.Sigmoid()))
        else:
            # self.net.append(Sequential(nn.Linear(hidden_features, out_features), nl))
            self.net.append(Sequential(nn.Linear(hidden_features, out_features)))

        # Original MLP using BatchLinear
        # self.net.append(Sequential(BatchLinear(in_features, hidden_features), nl))
        # for i in range(num_hidden_layers):
        #     self.net.append(Sequential(BatchLinear(hidden_features, hidden_features), nl))
        # if outermost_linear:
        #     self.net.append(Sequential(BatchLinear(hidden_features, out_features)))
        # else:
        #     self.net.append(Sequential(BatchLinear(hidden_features, out_features), nl))
...
''' The networks structure we used for fitting single image '''
class SingleBVPNet(nn.Module):
    '''A canonical representation network for a BVP.'''

    def __init__(self, out_features=1, type='sine', in_features=2,
                 mode='mlp', hidden_features=256, num_hidden_layers=3, out_nonlinear=False, **kwargs):
        super().__init__()
        self.mode = mode
        ...
        self.net = FCBlock(in_features=in_features, out_features=out_features, num_hidden_layers=num_hidden_layers,
                           hidden_features=hidden_features, outermost_linear=out_nonlinear, nonlinearity=type)
        print(self)
    ...
    def forward(self, model_input):
        ...
        # get the value ({x_i,y_i}_i) of key 'coords' as model input
