...
def get_mgrid(sidelen, dim=2):
    ...
    if dim == 2:
        ...
        pixel_coords -= 0.5
        pixel_coords *= 2.
        # pixel_coords = torch.Tensor(pixel_coords).squeeze(0).view(16, -1, dim)
        pixel_coords = torch.Tensor(pixel_coords).view(-1, dim)
        return pixel_coords
...
class Video(Dataset):
    def __init__(self, path_to_video, frame_num, frame_size):
        super().__init__()
        ...
        frame_num = frame_num
        # self.vid = self.vid[14:14 + frame_num, :, :, :]
        # self.vid = self.vid[22:22 + frame_num, :, :, :]
        self.vid = self.vid[200:200 + frame_num, :, :, :]
        # self.vid = self.vid[22:22 + frame_num, :, :, :]

        frame_size = frame_size

        self.vid_gray = np.zeros((self.vid.shape[0], frame_size, frame_size))
        for frame_idx in range(self.vid.shape[0]):
            # Convert RGB frame to grayscale using OpenCV function
            # gray_frame = cv2.cvtColor(self.vid[frame_idx, :frame_size, 300:300+frame_size, :], cv2.COLOR_RGB2GRAY)
            # gray_frame = cv2.cvtColor(self.vid[frame_idx, 0:0 + frame_size, 500:500 + frame_size, :], cv2.COLOR_RGB2GRAY)
            gray_frame = cv2.cvtColor(self.vid[frame_idx, 0:256, 250:550, :], cv2.COLOR_RGB2GRAY)

            gray_frame_downsample = cv2.resize(gray_frame, (frame_size, frame_size))

            # Store the grayscale frame in the grayscale video array
            # self.vid_gray[frame_idx, :, :] = gray_frame
            self.vid_gray[frame_idx, :, :] = gray_frame_downsample

        self.vid_gray = np.expand_dims(self.vid_gray, axis=-1)
        self.shape = self.vid_gray.shape[:-1]
        self.channels = self.vid_gray.shape[-1]

        self.vid = self.vid_gray
...
class Video_bee(Dataset):
    def __init__(self, path_to_video):
        super().__init__()

        self.vid = path_to_video
        self.vid = np.expand_dims(self.vid, axis=-1)
        self.shape = self.vid.shape[:-1]
        self.channels = self.vid.shape[-1]
...
class Camera(Dataset):
    def __init__(self, downsample_factor=1):
        ...
        self.img = Image.fromarray(skimage.data.camera()[32:32+128, 32:32+128])
        self.img_channels = 1
...
class Set11(Dataset):
    def __init__(self, Input_img, downsample_factor=1):
        super().__init__()
        self.downsample_factor = downsample_factor
        self.img = Image.fromarray(Input_img)
        self.img_channels = 1
...
class Implicit3DWrapper(torch.utils.data.Dataset):
    def __init__(self, dataset, sidelength=None, sample_fraction=1., frame_num=10, device='cpu', dtype='torch.float64'):
        ...
        data = (torch.from_numpy(self.dataset[0]).type(dtype) - 0.5) / 0.5
        # self.data = data.view(frame_num, -1)
        self.data = data.contiguous().view(frame_num, -1)
